...
class FloatingService : Service() {
    ...
    private fun createFloatingView() {
        // Outer container — bigger than content by BTN_HALF on each side
        outerContainer = object : FrameLayout(this) {
            override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
                super.onSizeChanged(w, h, oldw, oldh)
                outlineProvider = object : ViewOutlineProvider() {
                    override fun getOutline(view: View, outline: Outline) {
                        // Round only the content area, not the button overhang
                        outline.setRoundRect(BTN_HALF, BTN_HALF, w - BTN_HALF, h - BTN_HALF, CORNER_RADIUS)
                    }
                }
                clipToOutline = true
            }
        ...
        // WebView — centered inside outerContainer with BTN_HALF margin on each side
        ...
        val wvParams = FrameLayout.LayoutParams(FrameLayout.LayoutParams.MATCH_PARENT, FrameLayout.LayoutParams.MATCH_PARENT)
        wvParams.setMargins(BTN_HALF, BTN_HALF, BTN_HALF, BTN_HALF)
        outerContainer.addView(webView, wvParams)

        // 4 corner buttons — center at each corner of outerContainer
        addCornerButton(Gravity.START or Gravity.TOP, "▬", "Свернуть") { toggleMinimize() }
        addCornerButton(Gravity.END or Gravity.TOP, "✕", "Закрыть") { confirmClose() }
        addCornerButton(Gravity.START or Gravity.BOTTOM, "☰", "Взаимодействие", tag = "btn_interact") { toggleInteractionMode() }
        addCornerButton(Gravity.END or Gravity.BOTTOM, "⌨", "Клавиатура") { showKeyboardWindow() }

        // Window params — outer size = content + 2*BTN_HALF
        ...
        // Touch handling — only drag/pinch on the content area (not buttons)
    ...
    private fun addCornerButton(gravity: Int, text: String, desc: String, tag: String? = null, onClick: () -> Unit) {
    ...
    private fun updateInteractionMode() {
        ...
        // Update button 3 visual state (bottom-left)
